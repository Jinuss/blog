...
在vue3 + `Pinia`项目中，可以监听`store`的`actions`中的每个属性方法的执行。

## 正文

### 示例介绍

通过一个示例介绍如何监听`store`里`actions`的属性方法。
...
`userStore`中的`state`包含一个`name`属性，以及一个`setName`属性方法，用于设置`name`。
...
在组件中通过`$onAction`方法监听`action`，`$onAction`方法会接收一个回调函数`callback`作为参数，`callback`有四个参数，

- `name`：`actions`中的方法属性名，即方法名称
- `after`：在`action`调用完成后会触发，其`arg`参数为`action`方法的返回值
- `store`：即`userStore`，
- `onError`：在`action`调用时，若出现错误，则会触发
- `args`：表示`action`方法被调用时的参数数组

`after`和`onError`都是在`Pinia`源码中定义的方法，其作用是将参数推入到对应的事件队列中，等`action`被执行后，触发。

### 原理解析

### 订阅事件`$onAction`

`store.$onAction(callback)`可以理解成订阅`store`中的所有`action`事件，当`actions`中的事件被触发时，都会执行`callback`回调，而用`name`可以区分是哪一个具体的事件被触发。

在`Pinia`中的源码中，使用`defineStore`定义`store`时，返回的`store`就包含了`$onAction`属性，其实现如下：
...
`addSubscription`方法就是将监听`action`的回调`callback`推入到`actionSubscriptions`数组队列中，此外还定义了清除监听的函数`removeSubscription`。当若参数`detached`不存在或为`false`，则在作用域销毁时清理，最后返回清除函数。

即每个`store`中会有一个数组`actionSubscriptions`变量，它用于存放回调函数`callback`，当`action`被触发时就会遍历这个队列，执行回调。但是，在`store`的`actions`中定义事件（属性方法）时，就是很简单地写一个对象的属性方法。这是因为`Pinia`给这些属性方法重新包裹了一层。
...
`Pinia`定义`store`时，会对其`actions`对象进行`for...in`遍历，依次调用`action`方法进行包装，包装的过程就将`after`或`onError`中的回调放入到对应的队列里。`action`中最后返回的就是定义的`wrappedAction`方法，该方法就是用于封装属性方法的核心实现。

因此在如上组件中调用`userStore.setName`实际上执行的不是我们所定义的那个`setName`属性方法，而是经过`action`封装后的方法`wrappedAction`。
