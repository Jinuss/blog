...
`Pinia`可以在 vue2 和 vue3 中用于数据或状态的管理，同时 pinia 还提供了极其丰富的浏览器调试插件工具,更多详细内容可以参考 [Pinia官网](https://pinia.vuejs.org/zh)。
...
`createSetupStore` 函数是 Pinia 中的核心部分, 接收 6 个参数,分别是`id`,`setup`,`options`,`pinia`,`hot`,`isOptionsStore`。`createSetupStore`函数返回一个 store,这个 store 是一个对象,包含`$id`,`$state`,`$patch`,`$reset`,`$subscribe`,`$subscribeWith`,`$dispose`等方法。
...
`acceptHMRUpdate`函数接收两个参数，第一个参数是`store`，第二个参数是`hot`，即`import.meta.hot`，`acceptHMRUpdate`函数返回一个函数，这个函数接收一个参数，这个参数是`store`，这个函数会判断`store`是否是`import.meta.hot`的`module.$id`，如果是，则返回`store`，否则返回`null`。
